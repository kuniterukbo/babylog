...
・テストアカウントでログイン → トップページからから「アルバムを作る」を押下 → 情報を入力、「アルバムを作成する」を押下 → ルームが作成されます。  
...
#### ⚠️Herokuを利用して公開しているため、画像データは定期的に消去され表示されない可能性がございます。

# 課題点・今後の実装予定
・乳児は成長が早く、1ヶ月ごとで顔が変わってくる。  
　→**投稿した写真を月ごとやイベントごとにソートできる機能を追加する予定です。**  
・ルーム作成時のユーザーの選択で全てのユーザーが選択肢として表示されてしまう。  
　→**招待制に変更を検討してます。**

# 工夫した点
・ルームを削除したとき、そのルームに紐づく投稿のデータやコメントのデータも同時にテーブルから削除するように実装しました。  
・ルームの一覧はログインしているユーザーに紐付いているルームのみを表示するように実装しました。
